...
def my_predict_test(model, sql_data, table_data, tokenizer=None):
    model.eval()
    perm = list(range(len(sql_data)))
    st, ed = 0, 1
    with torch.no_grad():
        if isinstance(model, SQLBert):
            q_seq, col_seq, col_num, raw_q_seq, table_ids, header_type = to_batch_seq_test(sql_data, table_data, perm, st, ed, tokenizer=tokenizer)
